  ...
  implements ICodeSuggestionWebviewProvider
{
      ...
      switch (type) {
        ...
        case 'getAutofixDiffs': {
          try {
            ...
            void this.panel?.webview.postMessage({ type: 'setAutofixError' });
          }
        ...
        case 'applyGitDiff': {
          ...
          if (patchedContent === false) {
            ...
            const editor = vscode.window.visibleTextEditors.find(
              editor => editor.document.uri.toString() === `file://${fileUri}`,
            );

            if (!editor) return console.error('No editor found');
            const editorEndLine = editor.document.lineCount;
            edit.replace(
              vscode.Uri.parse(fileUri),
              new vscode.Range(new vscode.Position(0, 0), new vscode.Position(editorEndLine, 0)),
              patchedContent,
            );

            const success = await vscode.workspace.applyEdit(edit);
            if (success) {
              this.highlightAddedCode(fileUri, patch);
  ...
  private highlightAddedCode(fileUri: string, patch: string) {
    ...
    const codeBlocks = patch.split('\n').reduce<{ [startLine: string]: string[] }>((acc, line) => {
    ...
    let linesAddedInPrevBlocks = 0;
    for (const [blockStartLine, blockLines] of Object.entries(codeBlocks).sort(
      (a, b) => parseInt(a[0]) - parseInt(b[0]),
    )) {
      let linesAddedInThisBlock = 0;
      blockLines
        .filter(line => !line.startsWith('-'))
        .forEach((line, index) => {
          if (line.startsWith('+')) {
            addedLineNumbers.push(parseInt(blockStartLine) + index - 1 + linesAddedInPrevBlocks); // Subtract 1 because line numbers are zero-based
            linesAddedInThisBlock++;
          }
        });
      linesAddedInPrevBlocks += linesAddedInThisBlock;
    }
    ...
    const listeners: (NodeJS.Timeout | vscode.Disposable)[] = [];
    const removeHighlights = () => {
      editor.setDecorations(highlightDecoration, []);
      //cleanup listeners
      listeners.forEach(listener => {
        if ('dispose' in listener) listener.dispose();
        else clearTimeout(listener);
      });
    };
    listeners.push(setTimeout(removeHighlights, 30000));
    listeners.push(vscode.workspace.onDidChangeTextDocument(removeHighlights));
    listeners.push(vscode.workspace.onDidSaveTextDocument(removeHighlights));
  }
  ...
  <body>
      <div class="suggestion">
        ...
        <div class="tab-content is-selected sn-fix-content">
          ...
          <section class="ai-fix">
            ...
            <div class="sn-fix-wrapper">
              ...
              <div class="sn-loading is-hidden">
          ...
          <section class="sn-ai-fixes is-hidden">
          ...
          <section class="sn-ai-fix-error is-hidden">
            <div id="info-no-diffs" class="font-light">
        ...
        <section class="delimiter-top is-hidden">
