...
- **LS → `settings.json` (VS Code, optional):** `ConfigurationPersistenceService.persistInboundLspConfiguration` maps the global snapshot into VS Code settings and updates the last-known-value cache for each written key. It never writes to the explicit-overrides map — a structural guarantee (this code path has no access to that map), not a suppression check — so LS-originated writes can never be mistakenly recorded as user overrides.
- **IDE → LS (pull model):** `synchronize.configurationSection` triggers **`workspace/didChangeConfiguration`** with a flat VS Code settings payload whenever the `snyk` section changes. snyk-ls cannot parse this flat payload as **`LspConfigurationParam`**, so it falls back to the **pull model**: snyk-ls sends a **`workspace/configuration`** request, and the **`LanguageClientMiddleware`** responds with **`[{ settings: LspConfigurationParam }]`** — structured config with proper **`changed`** flags. Only settings marked as **explicitly changed** by the user (tracked via the **explicit-overrides map**, `ExplicitOverridesMap`) have **`changed: true`**; all others have **`changed: false`** so snyk-ls does not treat them as user overrides.
...
The **`LanguageClientMiddleware`** intercepts this request and builds a structured **`LspConfigurationParam`** response via **`serverSettingsToLspConfigurationParam`**, using **`ExplicitOverridesMap.getEntry()`** to set the **`changed`** flag on each setting — both a recorded explicit value and a pending reset sentinel count as changed. The middleware returns **`[{ settings: <LspConfigurationParam> }]`** (matching the **`[]DidChangeConfigurationParams`** shape snyk-ls expects).

**Explicit key tracking:** After the language client starts, **`LanguageServer`** registers **`workspace.onDidChangeConfiguration`** to mark which LS keys the user has explicitly changed (via **`markExplicitLsKeysFromConfigurationChangeEvent`**), which records the change in the same explicit-overrides map. The inbound-push path above has no access to this map at all, so LS-originated changes can never be recorded as user overrides — no suppression check needed.
